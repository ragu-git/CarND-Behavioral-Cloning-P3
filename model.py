...
def readDrivingLogs(dataPath, skipHeader=False):
    """
...
def findImages(dataPath):
    ...
    for directory in dataDirectories:
        lines = readDrivingLogs(directory)
        center = []
        left = []
        right = []
        measurements = []
...
def mergeImageData(center, left, right, measurement, correction):
    """
...
def createPreProcessingLayers():
    """
    Creates a model with the initial pre-processing layers.( Cropping image)
    """
    model = Sequential()
...
centerPaths, leftPaths, rightPaths, measurements = findImages('/home/workspace/data')
imagePaths, measurements = mergeImageData(centerPaths, leftPaths, rightPaths, measurements, 0.1)
...
# Train model for 10 epochs and a batch size of 32

       
model.fit_generator(train_generator, samples_per_epoch= 
                 int(len(train_samples)/32), validation_data=validation_generator,
                 nb_val_samples=int(len(validation_samples)/32), epochs=10, verbose=1,callbacks=[checkpoint, callback])
#print("Weights Saved")
